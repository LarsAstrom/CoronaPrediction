...
def main(do_plot=False,input_filename='input_data/input.csv',log_filename='log_parameter_tidsserie.csv',exp_weight=1,bugfix=False):
    ...
    alphas,betas,gammas,Ks,Ls,Kgammas = [],[],[],[],[],[]
    for num_days in range(14,len(a_inp)+1):
        print('Dag {} av {} startad.'.format(num_days,len(a_inp)))
        a,b,g,k,l,kg = corona.get_best_parameters(a_inp,d_inp,days=num_days,log_file=log_filename,exp_weight=exp_weight,bugfix=bugfix)
        alphas.append(a)
        betas.append(b)
        gammas.append(g)
        Ks.append(k)
